...
export function ReviewHeatmapFront() {
  ...
  const frontIssues = review.issues.filter(i => i.zone.view === 'front');

  // Phone dimensions in SVG units
  const SVG_W = 200;
  const SVG_H = Math.round(SVG_W * review.extractedHeight / review.extractedWidth);
        ...
        <div className="col-span-2 bg-white border border-slate-100 rounded-xl p-8 flex items-center justify-center">
          <div className="relative inline-block">
            <svg width={SVG_W} height={SVG_H} viewBox={`0 0 ${SVG_W} ${SVG_H}`}
              ...
              {/* Phone body */}
              <rect x="0" y="0" width={SVG_W} height={SVG_H} rx="16" ry="16" fill="#1e293b" />
              <rect x={SVG_W * 0.04} y={SVG_H * 0.04} width={SVG_W * 0.92} height={SVG_H * 0.92}
                rx="10" ry="10" fill="#334155" />
              {/* Issue overlays */}
              {frontIssues.map(issue => (
                <rect
                  key={issue.id}
