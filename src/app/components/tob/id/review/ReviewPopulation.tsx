...
export function ReviewPopulation() {
  ...
  function updatePop(patch: Partial<typeof pop>) {
    dispatch({ type: 'SET_ID_REVIEW_WIZARD', payload: { population: { ...pop, ...patch } } });
  }

  const canContinue = ageGroups.length > 0 && percentiles.length > 0 && gripHabits.length > 0 && useScenes.length > 0;

  function handleStart() {
    navigate('/tob/id/review/analyzing');
  }

  return (
    <div className="p-8 max-w-2xl mx-auto">
      <div className="mb-6">
        <div className="text-xs text-slate-400 mb-1">第 2 步 / 共 2 步 · {wiz.fileName}</div>
        <h1 className="text-xl font-semibold text-slate-800">配置评审人群</h1>
        <p className="text-sm text-slate-500 mt-1">指定评审目标人群，系统将据此进行握持适配性分析。</p>
      </div>
      ...
      <div className="flex justify-between mt-8">
        <button onClick={() => navigate('/tob/id/review/upload')}
